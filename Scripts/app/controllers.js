...
angular.module('healthCalc')
	.controller('HealthController', ['$scope', '$timeout', function ($scope, $timeout) {
		...
		$scope.callbackForm_1 = false;
		$scope.callbackForm_2 = false;
		$scope.showCallback = function (name) {
		...
		$scope.includeInsurer = function () {
			if ($scope.insurer.includeSelf) {
				...
				$scope.insuredPerson[0].birthday = Object.prototype.toString.call($scope.insurer.birthday) === "[object Date]" ? $scope.insurer.birthday.toLocaleString() : $scope.insurer.birthday;
			} else {
		...
		$scope.includeInsurerField = function (name) {
			if ($scope.insurer.includeSelf) {
				var value = $scope.insurer[name];
				if (name == 'birthday') {
					$scope.insuredPerson[0][name] = Object.prototype.toString.call(value) === "[object Date]" ? value.toLocaleString() : value;
				} else {
					$scope.insuredPerson[0][name] = value;
				}
			}
		}
